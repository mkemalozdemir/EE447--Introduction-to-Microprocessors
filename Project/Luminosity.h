...
extern void isBusy(void);
extern char readBYTE(int slave_address, char slave_command);
extern char* putArray(void);
...
int luminosity(void){ // function gives the lux value
	...
	for(i=0; i<512;){
			channel0 = 256*array[i+1] + array[i];			// read datalow0 and datahigh0 from the memeory for each sample
			channel1 = 256*array[i+3] + array[i+2];		// read datalow1 and datahigh1 from the memeory for each sample
			sum += CalculateLux(0,2,channel0,channel1,0);	//call the function to calculate lux value
			i +=4;																		
	}
	avg = sum/128; // calculate the average of the array
	return avg;
}
...
//Lux calculation code from datasheet of TSL2561
