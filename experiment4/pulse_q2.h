...
void portB_init(void){	//Port B initialization
	SYSCTL->RCGCGPIO |= (1<<1);	// Set system clock for Port B
	__ASM("NOP");
	__ASM("NOP");
	__ASM("NOP");
	
	GPIOB->DIR &= ~(1<<4);	// PB4 is set as input
	GPIOB->AFSEL |= (1<<4);	//set PB4 as alternative function
	GPIOB->PCTL &=~(15<<16);//Clear bits to choose alternative function
	GPIOB->PCTL |=(7<<16); // Arrange PB4 as TIMER1A
	GPIOB->AMSEL = 0;	//Clear analog signals
	GPIOB->DEN |=(1<<4);	// Set PB4 for digital inputs
	
	SYSCTL->RCGCTIMER |=(1<<1);	//Set system clock for TIMER1A
	__ASM("NOP");
	__ASM("NOP");
	__ASM("NOP");
	...
	TIMER1->TAILR =60000; //
	TIMER1->CTL |= (1<<3)|(1<<2);//Both edge arrangement
...
void printer(int number){
	...
	int i=0;
	char holder[10];	// declare holder string to hold digits of the number as character
	char string[10];	
	
	for(i; number>0;i++){
		holder[i] = number%10 + 48;	//First digit of the number is inserted in holder as ascii representation
		number = number/10;	// Decrease 1 digit
	}
	int j=0;
	
	for (i=i-1;i>=0;i--){
		string[j] = holder[i];		//Reverse the holder string
		j++;
	}
	string[j] ='\r';
	string[j+1] ='\4';
